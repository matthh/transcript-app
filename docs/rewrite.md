...
**Date**: 2026-02-24
**Updated**: 2026-02-24 (post-Phase 5)
...
Two-layer detection ensures both intelligent classification and deterministic coverage:

**Layer 1 — LLM classifier**: The existing Haiku classifier gets a new `searchStrategy` output field. Prompt examples teach it to recognize aggregation, frequency, cross-episode pattern, and cultural reference queries.

**Layer 2 — Deterministic overrides**: Regex patterns that always force agent routing, regardless of classifier output. Ensures known-hard patterns never miss:

```regex
/\b(catchphrase|recurring phrase|always says|signature line|running joke)\b/i
/\b(how often|how many times|every time|most frequent)\b/i
/\b(who says .* more|says .* the most)\b/i
/\b(all the times|every mention|across (?:all )?episodes)\b/i
```

**Interaction with Phase 5 supplemental queries**: The existing supplemental query system (generated by the Haiku classifier for persona/aggregation patterns) runs within the RAG pipeline. When a query is routed to the agent path, the supplemental query system is bypassed entirely — the agent does its own iterative search instead. There is no conflict; these are mutually exclusive paths.
...
**Phase A — Types and routing (small)**:
- Add `searchStrategy` to `ClassificationResult` type
- Add `shouldUseAgentSearch()` to routing policy with deterministic overrides
- Add agent constants (`AGENT_SEARCH_MODEL`, `AGENT_MAX_ITERATIONS`)
...
- Update query logger for `'agent_search'` routing path
...
Transcripts must be available in the Vercel serverless function for filesystem-based grep.

**Option 1 (preferred): Bundle via `outputFileTracingIncludes`**
...
**Option 2 (fallback): Load from Vercel Blob**
- If bundle size is an issue, load transcripts on-demand from Blob storage
...
- **Agent queries**: ~$0.10-0.50/query (5-15 Sonnet tool-use turns + grep result tokens). System is not heavily used, so total cost impact is negligible.
...
| Agent tool calls exceed Vercel timeout | Low | Medium — partial answer | `AGENT_MAX_ITERATIONS = 15` caps iterations; Vercel `maxDuration` gives headroom; force final answer on max iterations |
...
1. **Routing overlap with Phase 5**: The deterministic agent patterns (e.g., "catchphrase", "how often") overlap with Phase 5's supplemental query triggers. Should agent routing supersede these (cleaner, agent handles the whole class) or only activate for patterns *not* already covered (lower risk, incremental)?
2. **Phase 5 deprecation path**: Once the agent path is proven, should we remove the bespoke catchphrase/personal-aside sub-chunks and supplemental query machinery? Or keep them as a fast-path optimization for known patterns?
3. **Transcript bundling**: Should we verify Vercel bundle size before committing to filesystem approach, or build with Blob fallback from the start?
4. **Agent model**: Sonnet is proposed for reasoning quality. Would Haiku be sufficient for the tool-use loop (cheaper, faster), with Sonnet only for final synthesis?
5. **Max iterations**: 15 is proposed as a cap. Should we start lower (e.g., 8) and increase if needed?
6. **Streaming UX**: Should agent queries show a different UI state (e.g., "Deep searching...") vs RAG queries ("Searching...")?
