...
import { logQuery as rawLogQuery, generateLogId, QueryLogEntry } from '@/lib/query-logger';
...
export async function runSearch(input: SearchInput): Promise<SearchResult> {
  const { query, limit: rawLimit, offset: rawOffset, variant, depth: rawDepth } = input;
  ...
  const logTag = {
    ...
    ...(input.externalKeyId ? { externalKeyId: input.externalKeyId } : {}),
  };
  const logQuery = (data: Omit<QueryLogEntry, 'id' | 'timestamp'>, id?: string) =>
    rawLogQuery({ ...data, ...logTag }, id);
